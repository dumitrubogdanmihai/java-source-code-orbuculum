...
curl -X POST -H 'Content-type:application/json' --data-binary '{"add-field": {"name":"line-start", "type":"text_general", "multiValued":false, "stored":true}}' http://localhost:8983/solr/orbuculum/schema
curl -X POST -H 'Content-type:application/json' --data-binary '{"add-field": {"name":"line-end", "type":"text_general", "multiValued":false, "stored":true}}' http://localhost:8983/solr/orbuculum/schema
